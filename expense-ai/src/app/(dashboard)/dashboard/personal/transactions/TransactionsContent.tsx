...
export default function TransactionsContent() {
  ...
  const stepMonth = (dir: 1 | -1) => {
    ...
    setMonth(newMonth)
    setYear(newYear)
    router.replace(`/dashboard/personal/transactions?month=${newMonth}&year=${newYear}`, { scroll: false })
  }
      ...
      <div className="px-6 pt-8 pb-6 max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <Link
            ...
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl border border-slate-100 bg-white shadow-sm text-slate-500 hover:text-slate-700 hover:bg-slate-50 transition-colors"
          >
            <ArrowLeft className="h-4 w-4" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-slate-900">{t("personal.transactions.title")}</h1>
            <p className="text-sm text-slate-500 mt-0.5">{t("personal.transactions.subtitle")}</p>
          </div>
        </div>
      ...
      <div className="px-6 pb-6 max-w-4xl mx-auto">
        {isLoading ? (
          ...
          <div className="rounded-3xl border border-slate-100 bg-white px-6 py-16 text-center shadow-sm">
            ...
            <h3 className="font-bold text-slate-900">{t("personal.transactions.empty.title")}</h3>
            <p className="mt-1 text-sm text-slate-400">{t("personal.transactions.empty.description")}</p>
          </div>
        ) : (
          <Card className="rounded-[2rem] border-slate-100 bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.25em] text-slate-400">
                {t("personal.transactions.count", { count: transactions.length })}
              </p>
            </div>
